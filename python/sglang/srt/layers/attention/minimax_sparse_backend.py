...
class MiniMaxSparseAttnBackend(AttentionBackend):
    def __init__(self, runner: ModelRunner):
        ...
        if self.is_npu:
            self.use_msa = False
            # Eagerly prime the native sparse main-op probe so that `import
            # attentions` (which sets ASCEND_CUSTOM_OPP_PATH) runs BEFORE
            # cuda-graph capture -- a lazy probe inside capture sets the env too
            # late (the original "must export ASCEND_CUSTOM_OPP_PATH" bug). No-op
            # when both native gates are off; the wrapper still re-reads the gate
            # per call to pick native vs Triton.
            ...
            if _native_sparse_decode_enabled() or _native_verify_enabled():
        ...
        # MSA decode is not cuda-graph-safe (wrong results compound across replays):
        # use it only when decode does NOT run under cuda graph; else route decode
        # through the triton sparse path. Decide from the resolved cuda_graph_config
        # (same source as init_decode_cuda_graph), not legacy disable_* flags.
