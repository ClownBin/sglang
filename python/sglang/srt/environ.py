...
class Envs:
    ...
    # the Triton split-K kernel. Auto-detect (no on/off gate): the backend probes
    # torch.ops.npu.npu_sparse_attention_score (statically linked into
    # libsgl_kernel_npu.so) at init; when available it routes to the ascendc op,
    # otherwise it falls back to the Triton split-K path.
    # Optional path to a vllm_ascend_C.so build of the aclnn op wrapper; used as a
    # fallback only when the native sgl-kernel-npu op is unavailable AND it points
    # to a non-libPTAExtensionOPS .so. Unset -> native auto-detect only.
